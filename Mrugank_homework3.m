...
mfit_curves = NaN(nsubs, size(PSEs, 2), size(PSEs, 2), len(cohLevs));
...
%         end
        ...
        for ii = 1:4
            mfit_curves(sub, modelN, ii, :) = psyfuncGenerator(cohLevs, [0 estP_M1]);
        end
...
figure();
for ii = 1:4
    subplot(4, 4, 4*(ii-1)+1); hold on;
    for ttype = 1:size(PSEs, 2)
        this_data = data(modelN, ttype, :, sub, :);
        plot(cohLevs, squeeze(mean(this_data, 2)), color_vect(ttype));%, 'DisplayName', legnames{ttype})
    end
    subplot(4, 4, 4*(ii-1)+2); hold on;
    for ttype = 1:size(PSEs, 2)
        this_data = data(modelN, ttype, :, sub, :);
        plot(cohLevs, squeeze(mean(this_data, 2)), color_vect(ttype));%, 'DisplayName', legnames{ttype})
    end
    subplot(4, 4, 4*(ii-1)+3); hold on;
    for ttype = 1:size(PSEs, 2)
        this_data = data(modelN, ttype, :, sub, :);
        plot(cohLevs, squeeze(mean(this_data, 2)), color_vect(ttype));%, 'DisplayName', legnames{ttype})
    end
    subplot(4, 4, 4*(ii-1)+4); hold on;
    for ttype = 1:size(PSEs, 2)
        this_data = data(modelN, ttype, :, sub, :);
        plot(cohLevs, squeeze(mean(this_data, 2)), color_vect(ttype));%, 'DisplayName', legnames{ttype})
    end
